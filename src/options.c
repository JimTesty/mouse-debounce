...
void options_print_usage(const char *argv0) {
    fprintf(stderr,
        ...
        "  if press duration < short0, that Up uses only hold0 (hold is ignored)\n"
        "  otherwise, that Up uses only hold (hold0 is ignored); short0=0 uses hold\n"
        "  hold0 and hold must be greater than 0\n"
        "\n"
        "Sound:\n"
        "  --sound-volume N   alert/debug tick volume, 0..1 (default 0.1; 0 mutes)\n"
        "  --debug            play startup, click, drag, and filter diagnostic tones\n"
