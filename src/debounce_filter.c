...
static bool schedule_timer(DebounceFilter *filter, MouseButton button) {
    ...
    CFAbsoluteTime fire = CFAbsoluteTimeGetCurrent() + (double)filter->hold_ns / 1e9;
...
void debounce_filter_init(
    ...
    filter->short_ns = ms_to_ns(short_ms);
    filter->hold_ns = ms_to_ns(hold_ms);
...
CGEventRef debounce_filter_handle(
    ...
    DebounceAction action = debounce_on_up(
        &runtime->logic, now_ns, filter->short_ns, filter->hold_ns
    );
